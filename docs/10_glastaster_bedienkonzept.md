...
4. Oben bei **Standbyanzeige** **einzeln im Wechsel** wählen. Unter **Standbyanzeige bei Tag** die **Statuselemente 1–3** auf `Statuswert 1`, `Statuswert 2` und `Statustext 2` stellen; Element 4 bleibt nicht aktiv. Der ETS-Screenshot vom 25.09.2026 zeigt hier noch `Uhrzeit` und `Interne Temperatur`; diese beiden Felder daher umstellen. Eine Wechselzeit festlegen, zum Beispiel die gezeigten 5 Sekunden. **Standbyanzeige bei Nacht** kann auf **Verhalten wie Tag** bleiben.
